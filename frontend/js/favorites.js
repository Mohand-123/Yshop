// Quand la page est chargée, on affiche les favoris et on met à jour les badges
...
// Charge les produits favoris depuis l'API puis les affiche dans la grille
async function chargerFavoris() {
  const ids = getFavoris();
  const grid = document.getElementById('favorites-grid');

  // Si aucun favori → message vide
  if (ids.length === 0) {
    grid.innerHTML = '<p class="empty-msg">Vous n\'avez aucun favori.</p>';
    return;
  }

  // On récupère tous les produits depuis l'API
  const tous = await getAllProduits();
  const favoris = [];

  // On garde seulement ceux dont l'id est dans la liste des favoris
  ...
  // On affiche les cartes produits
  grid.innerHTML = '';
  for (let i = 0; i < favoris.length; i++) {
    ...
    grid.innerHTML +=
      ...
      // Nom + prix + lien vers la page produit
      ...
      // Boutons : ajouter au panier + retirer des favoris
...
// Supprime un produit des favoris puis recharge la liste
function supprimerFavori(id) {
  const favoris = getFavoris();
  const newFavoris = [];

  // On garde tous les favoris sauf celui à supprimer
  for (let i = 0; i < favoris.length; i++) {
...
// Met à jour les badges (favoris + panier) dans l'en‑tête
